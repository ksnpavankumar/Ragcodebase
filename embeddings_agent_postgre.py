...
# PostgreSQL Connection Credentials from Colab Secrets
...
copilot_tools = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Search Jira tickets by keywords in the title (summary) or description.",
            "parameters": {
                "type": "object",
                "properties": {
...
@app.get("/")
def home():
    return {"status": "Enterprise Engineering Knowledge Copilot API with pgvector is running!"}
...
def run_copilot(req: QueryRequest):
    try:
        ...
        # ==========================================
        # Step 3: Query PostgreSQL Vector Database (pgvector)
        # ==========================================
        ...
        # Connect to PostgreSQL and query nearest neighbors using cosine distance (<=>)
        conn = psycopg2.connect(
            host=PG_HOST, port=PG_PORT, database=PG_DATABASE, user=PG_USER, password=PG_PASSWORD
        )
        cursor = conn.cursor()

        # Format vector list as string representation for pgvector query
        ...
        for idx, row in enumerate(search_results, start=1):
            source_file, source_type, linked_keys, content = row
            sources_used.append({
                "file": source_file,
                "type": source_type,
                "jira_links": linked_keys
            })
            context_snippets.append(
                f"--- Source [{idx}]: {source_file} ---\n{content}"
            )
        ...
        return {
            ...
            "data_source": "hybrid_apis_and_pgvector" if tool_context_string else "pgvector_search_only"
        }
...
print("\n" + "=" * 60)
print(f"🚀 FastAPI Full-Stack Hybrid Copilot with pgvector Live!")
